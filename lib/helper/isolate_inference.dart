...
class IsolateInference {
  ...
  static void entryPoint(SendPort sendPort) async {
    ...
    await for (final InferenceModel isolateModel in port) {
      ...
      final output = [
        List.generate(
          isolateModel.outputShape[1], // This is 24
              (_) => List<double>.filled(isolateModel.outputShape[2], 0.0), // This is 8400
        )
      ];
      // // Run inference
      ...
      final List<List<double>> resultList = output.first as List<List<double>>;

// Flatten the nested list (assuming you want to sum all elements across all inner lists)
      final List<double> result = resultList.expand((e) => e).toList();

// Calculate the total score (maxScore)
      final double maxScore = result.fold(0.0, (sum, value) => sum + value);

// Initialize the classification map
      final Map<String, double> classification = {};

      if (maxScore > 0) {
        final int minLength = result.length < isolateModel.labels.length
            ? result.length
            : isolateModel.labels.length;

        for (var i = 0; i < minLength; i++) {
          if (result[i] > 0) {
            // Normalize score and map to the corresponding label
            classification[isolateModel.labels[i]] = result[i] / maxScore;
            print("${isolateModel.labels[i]}   ${result[i]}");
          }
        }
      }

// Send the classification map back through the isolate's response port
